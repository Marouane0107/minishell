...
/*   Updated: 2023/08/26 00:30:41 by otamrani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

void	ft_free1(char **s1, char **s2)
{
	if (s1 && *s1)
	{
		free(*s1);
...
char	*ft_chkline(char **buffer1, ssize_t j)
{
	...
	if (j < 0)
		return (ft_free1(buffer1, &rest), NULL);
	tmp = rest;
	...
	if (sline)
	{
		...
		return (ft_free1(&tmp, buffer1), sline);
	}
	if (j == 0)
	{
		if (rest && *rest)
			sline = ft_strdup(rest);
		return (ft_free1(&rest, buffer1), sline);
	}
	return (0);
}
...
char	*get_next_line(int fd)
{
	...
	while (i >= 0)
	{
		...
		if (!result && !buffer)
			return (0);
		if (result && *result)
			return (ft_free1(0, &buffer), result);
		i = read(fd, buffer, BUFFER_SIZE);
	}
	return (0);
}
